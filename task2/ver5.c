        ...
        for (j = i + 1; j < N; j++) {
            ...
            for (k = i + 1; k < N; )
                if (k < max((SIZE - BLKSIZE), 0)) {
                    ...
                    A[IDX(j, k + 1, SIZE)] -= tmp2 * A[IDX(i, k + 1, SIZE)];
                    A[IDX(j, k + 2, SIZE)] -= tmp2 * A[IDX(i, k + 2, SIZE)];
                    A[IDX(j, k + 3, SIZE)] -= tmp2 * A[IDX(i, k + 3, SIZE)];
                    A[IDX(j, k + 4, SIZE)] -= tmp2 * A[IDX(i, k + 4, SIZE)];
                    A[IDX(j, k + 5, SIZE)] -= tmp2 * A[IDX(i, k + 5, SIZE)];
                    A[IDX(j, k + 6, SIZE)] -= tmp2 * A[IDX(i, k + 6, SIZE)];
                    A[IDX(j, k + 7, SIZE)] -= tmp2 * A[IDX(i, k + 7, SIZE)];
                    k += BLKSIZE;
                } else {
                    A[IDX(j, k, SIZE)] -= tmp2 * A[IDX(i, k, SIZE)];
                    k++;
